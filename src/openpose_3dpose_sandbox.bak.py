...
def read_openpose_json_mydata(smooth=True, *args):
    ...
    smoothed = {}
    ### extract x,y and ignore confidence score
    _file = os.path.join(openpose_output_dir, json_files[0])
    if not os.path.isfile(_file): raise Exception("No file found!!, {0}".format(_file))
    data = json.load(open(_file))
    # take first person
    # my data format
    for key in range(10, len(data["pose_keypoints_2d"]), 10):
        _data = data["pose_keypoints_2d"][str(key)]
...
def main(_):
    ...
    with tf.Session(config=tf.ConfigProto(device_count=device_count, allow_soft_placement=True)) as sess:
        ...
        for n, (frame, xy) in enumerate(smoothed.items()):
            ...
            all_poses_3d = []
            enc_in = data_utils.unNormalizeData2d(enc_in, data_mean_2d, data_std_2d, dim_to_use_2d)
