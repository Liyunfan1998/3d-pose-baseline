...
not_use_joints = ['', 'Neck/Nose', 'LKnee', 'LFoot', 'RKnee', 'RFoot']
...
def load_stacked_hourglass(data_dir, subjects, actions):
    ...
    for subj in subjects:
        for action in actions:
            ...
            dpath = os.path.join(data_dir, 'S{0}'.format(subj), '{0}*.h5'.format(action))
            print(dpath)
            ...
            for fname in fnames:
                ...
                if seqname.startswith(action):
                    ...
                    with h5py.File(fname, 'r') as h5f:
                        ...
                        dim_to_use_x = np.where(np.array([x not in not_use_joints for x in H36M_NAMES]))[0] * 2
...
def unNormalizeData2d(normalized_data, data_mean, data_std, dimensions_to_use):
